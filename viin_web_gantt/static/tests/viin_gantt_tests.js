	...
	QUnit.module('Views', {
		beforeEach: function() {
			...
			this.data = {
				task: {
					...
					records: [
						{
                            id: 1, user_id: 4, manager_id: 4, name: 'Room 2: Decoration1', 
						...
						{
                            id: 3, user_id: session.uid, manager_id: 4, name: 'Decoration', 
						...
						{
                            id: 4, user_id: 4, manager_id: 4, name: 'Decoration 1', 
						...
						{
                            id: 5, user_id: 4, manager_id: 4, name: 'Decoration 2', 
						...
						{
                            id: 6, user_id: 4, manager_id: 4, name: 'Decoration 4', 
			...
			var viin_gantt = await createViinGanttView({
				...
				mockRPC: function (route, args) {
	                if (args.method == 'write_from_gantt') {
	                    ...
	                    assert.deepEqual(id, 6, "should be id of 6 update when move the task");
	   ...
	   QUnit.test('TC01: simple viin gantt rendering', async function (assert) {
		...
		QUnit.test('TC02: test when click scale button', async function (assert) {
			...
			assert.containsN(viin_gantt, '.gdfTable:not(.ganttFixHead) .taskEditRow:not(.emptyRow)', 5,
	            "should 5 row gantt when scale is month");
			...
			assert.containsN(viin_gantt, '.gdfTable:not(.ganttFixHead) .taskEditRow:not(.emptyRow)', 6,
	            "should 6 row gantt when scale is year");
			...
			assert.containsN(viin_gantt, '.gdfTable:not(.ganttFixHead) .taskEditRow:not(.emptyRow)', 7,
	            "should 7 row gantt when scale is all");
			assert.strictEqual($('.focus-date-breadcrumb').text(),
		...
		QUnit.test('TC05: when create task by insert above button', async function(assert) {
			...
			await timeout(1000);
			await testUtils.fields.editInput(viin_gantt.$('#tid_5').prev().find('input[name=name]'), 'task 4 created add above');
			await timeout(1500);
			viin_gantt.$('#tid_5').prev().find('input[name=name]').change();
			await timeout(3000);
			viin_gantt.destroy()
		});
		...
		QUnit.test('TC06: when edit task', async function(assert) {
			...
			await testUtils.dom.click(viin_gantt.$('#tid_5'));
			await timeout(500);
			await testUtils.fields.editInput(viin_gantt.$('#tid_5').find('input[name=name]'), 'Decoration 2.1');
			viin_gantt.$('#tid_5').find('input[name=name]').change();
			await timeout(3000);
			viin_gantt.destroy()
		});
		...
		QUnit.test('TC06: when edit task by form', async function(assert) {
			...
			await testUtils.dom.triggerMouseEvent(viin_gantt.$('#tid_5'), 'dblclick');
		...
		QUnit.test('TC10: when delete task by open form task', async function(assert) {
			...
			var viin_gantt = await createViinGanttView({
				...
				mockRPC: function (route, args) {
	                if (args.method == 'unlink') {
						var id = args.args[0];
	                    assert.deepEqual(id, 6, "Should delete task 'Decoration'");
                        return Promise.resolve();
	                }
	                return this._super.apply(this, arguments);
	            },
		...
		QUnit.test('TC14: when filter task by assigning to current user',  async function(assert) {
			...
			await timeout(3000);
			await testUtils.dom.click($('.btn-group.o_dropdown').first().find('.o_dropdown_toggler_btn'));
			await testUtils.dom.click($('.o_filter_menu.show .o_dropdown_menu.show .o_menu_item').first().find('a'));
			await timeout(1000);
		...
		QUnit.test('T15: when group task by assignee',  async function(assert) {
			...
			var archs = {
				...
				'<search>'+
					...
					'<group string="Group By User id">'+
                        '<filter name="user_id" string="Assignee" context="{\'group_by\': \'user_id\'}"/>' + 
                    '</group>'
				+'</search>' 
			...
			await timeout(3000);
			await testUtils.dom.click($('.btn-group.o_dropdown').eq(1).find('.o_dropdown_toggler_btn'));
			await testUtils.dom.click($('.o_group_by_menu.show .o_dropdown_menu.show .o_menu_item').first().find('a'));
			await timeout(2000);
			$('.gdfTable:not(.ganttFixHead) .taskEditRow:not(.emptyRow)').each(function(i, elem) {
				if(i == 0) {
